<x-app-layout>
  <x-slot name="header">
    <div class="flex items-center justify-between">
      <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-100">New user</h2>
  ...
  <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
    <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
      <form method="POST" action="{{ route('admin.users.store') }}" class="grid grid-cols-1 gap-6 sm:grid-cols-2">
        @csrf

        <div class="sm:col-span-2">
          <x-input-label value="Full name" />
          <x-text-input type="text" name="full_name" class="mt-1 block w-full" required value="{{ old('full_name') }}" />
          <x-input-error :messages="$errors->get('full_name')" class="mt-2" />
        </div>

        <div>
          <x-input-label value="Email" />
          <x-text-input type="email" name="email" class="mt-1 block w-full" required value="{{ old('email') }}" />
          <x-input-error :messages="$errors->get('email')" class="mt-2" />
        </div>

        <div>
          <x-input-label value="Phone" />
          <x-text-input type="text" name="phone" class="mt-1 block w-full" value="{{ old('phone') }}" />
        ...
        <div>
          ...
          <select name="role" required
            ...
            @foreach($roles as $r)
              <option value="{{ $r->value }}" @selected(old('role')===$r->value)>{{ ucfirst($r->value) }}</option>
            @endforeach
          </select>
        ...
        <div>
          ...
          <x-text-input type="password" name="password_confirmation" class="mt-1 block w-full" required />
        </div>
